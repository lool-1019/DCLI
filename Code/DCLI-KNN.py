...
# CL方法 - 单网络置信学习
def get_noise_cl(label_new, pre_new, xall_new, X_test, y_test, seed_t):
    ...
    y_train2 = label_1.astype(np.int32)
    ...
    log_reg = KNeighborsClassifier(n_neighbors=5)
    ...
    while t_loc < loc_20:
        ...
        il = il + 1
    eff = count_effort / count_t1 if count_t1 > 0 else 0
...
def dual_confident_learning(label_new, psx_A, psx_B, xall_new, X_test, y_test, seed_t):
    ...
    if n_unlabeled > 0:
        ...
        clf_A_temp = KNeighborsClassifier(n_neighbors=5)
        clf_A_temp.fit(xall_new_A, label_new_A)
        
        clf_B_temp = KNeighborsClassifier(n_neighbors=5)
        clf_B_temp.fit(xall_new_B, label_new_B)
...
csv_order = {0: 'activemq', 1: 'camel', 2: 'derby', 3: 'geronimo', 4: 'hbase', 
             5: 'hcommon', 6: 'mahout', 7: 'openjpa', 8: 'pig', 9: 'tuscany'}


def con_learn():
    file_name = 'KNN-comparison.csv'
    ...
    for i in range(10):
        ...
        # 存储所有方法的结果
        yor_all = [[] for _ in range(4)]  # ori, cl, cli, dcli
        auc_all = [[] for _ in range(4)]
        prec_all = [[] for _ in range(4)]
        recall_all = [[] for _ in range(4)]
        mcc_all = [[] for _ in range(4)]
        effort_all = [[] for _ in range(4)]
        ...
        for ix in range(10):
            ...
            y_or = [[] for _ in range(4)]
            auc_or = [[] for _ in range(4)]
            prec_or = [[] for _ in range(4)]
            recall_or = [[] for _ in range(4)]
            mcc_or = [[] for _ in range(4)]
            effort_or = [[] for _ in range(4)]
            ...
            for train_index, test_index in sfolder.split(ob, label):
                ...
                psx_cli = np.zeros((len(y_train), 2))
                ...
                for k, (cv_train_idx, cv_holdout_idx) in enumerate(kf.split(X_train, y_train)):
                    ...
                    psx_cli[cv_holdout_idx] = psx_cv1
                ...
                for k, (cv_train_idx, cv_holdout_idx) in enumerate(kf.split(X_train, y_train)):
                    ...
                    log_reg = KNeighborsClassifier(n_neighbors=6)
                    log_reg.fit(X_resampled, y_resampled)
                    psx_cv = log_reg.predict_proba(X_holdout_cv)
                ...
                f1_ori = metrics.f1_score(y_test_1, pre1, pos_label=1, average="binary")
                fpr, tpr, thersholds = metrics.roc_curve(y_test_1, pre1)
                auc_ori = metrics.auc(fpr, tpr)
                prec_ori = metrics.precision_score(y_test_1, pre1, pos_label=1)
                recall_ori = metrics.recall_score(y_test_1, pre1, pos_label=1)
                mcc_ori = matthews_corrcoef(y_test_1, pre1)
                ...
                while t_loc < loc_20:
                    ...
                    if sorted_data[il][2] == 1:
                        count_effort = count_effort + 1
                    il = il + 1
                effort_ori = count_effort / count_t1 if count_t1 > 0 else 0
                
                y_or[0].append(f1_ori)
                auc_or[0].append(auc_ori)
                prec_or[0].append(prec_ori)
                recall_or[0].append(recall_ori)
                mcc_or[0].append(mcc_ori)
                effort_or[0].append(effort_ori)
                
                # 方法1: CL
                f1_cl, auc_cl, prec_cl, recall_cl, mcc_cl, eff_cl = get_noise_cl(
                    y_train, psx, X_train, X_test, y_test, seed[ix]
                )
                y_or[1].append(f1_cl)
                auc_or[1].append(auc_cl)
                prec_or[1].append(prec_cl)
                recall_or[1].append(recall_cl)
                mcc_or[1].append(mcc_cl)
                effort_or[1].append(eff_cl)
                
                # 方法2: CLI
                f1_cli, auc_cli, prec_cli, recall_cli, mcc_cli, eff_cli = get_noise_cl(
                    y_train, psx_cli, X_train, X_test, y_test, seed[ix]
                )
                y_or[2].append(f1_cli)
                auc_or[2].append(auc_cli)
                prec_or[2].append(prec_cli)
                recall_or[2].append(recall_cli)
                mcc_or[2].append(mcc_cli)
                effort_or[2].append(eff_cli)
                
                # 方法3: DCLI
                f1_dcli, auc_dcli, prec_dcli, recall_dcli, mcc_dcli, eff_dcli = dual_confident_learning(
                    y_train, psx_A, psx_B, X_train, X_test, y_test, seed[ix]
                )
                y_or[3].append(f1_dcli)
                auc_or[3].append(auc_dcli)
                prec_or[3].append(prec_dcli)
                recall_or[3].append(recall_dcli)
                mcc_or[3].append(mcc_dcli)
                effort_or[3].append(eff_dcli)
            
            # 计算每个seed的平均值
            for method_idx in range(4):
                yor_all[method_idx].append(np.mean(y_or[method_idx]))
                auc_all[method_idx].append(np.mean(auc_or[method_idx]))
                prec_all[method_idx].append(np.mean(prec_or[method_idx]))
                recall_all[method_idx].append(np.mean(recall_or[method_idx]))
                mcc_all[method_idx].append(np.mean(mcc_or[method_idx]))
                effort_all[method_idx].append(np.mean(effort_or[method_idx]))
        
        # 计算所有seed的总平均值并写入CSV
        for method_idx in range(4):
            ori_all.append(np.mean(yor_all[method_idx]))
        for method_idx in range(4):
            ori_all.append(np.mean(auc_all[method_idx]))
        for method_idx in range(4):
            ori_all.append(np.mean(prec_all[method_idx]))
        for method_idx in range(4):
            ori_all.append(np.mean(recall_all[method_idx]))
        for method_idx in range(4):
            ori_all.append(np.mean(mcc_all[method_idx]))
        for method_idx in range(4):
            ori_all.append(np.mean(effort_all[method_idx]))
